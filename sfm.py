...
def get_reprojection_residuals(points2d: np.ndarray, points3d: np.ndarray, intrinsics: np.ndarray,
    ...
    """ YOUR CODE HERE """
    #将3d点转换为齐次坐标
    ho_3d_points = np.concatenate((points3d, np.ones((points3d.shape[0], 1))), axis=1)
    ho_3d_points_T = np.transpose(ho_3d_points)
    ...
    #将3d点投影到2d平面相关
    calculated_pts2d = np.matmul(P, ho_3d_points_T)
...
def add_points3d(image_id1: str, image_id2: str, all_extrinsic: dict, intrinsics, points3d: np.ndarray,
    ...
    """
    #新3d点
    new_points3d = triangulate(image_id1 = image_id1, image_id2 = image_id2, kp_idxs1 = matches[:, 0], kp_idxs2 = matches[:, 1], extrinsics1 = all_extrinsic[image_id1], extrinsics2 = all_extrinsic[image_id2], intrinsics = intrinsics)
